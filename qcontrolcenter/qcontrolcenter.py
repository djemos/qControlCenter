...
# the .desktop files in that category (folder in CONFIG_DIR).
	...
	def cellBuilder(self, iconName, programName, commentName, execName):
		cell = (
				...
				u"<a href='" + (unicode(execName).rstrip("%u")).rstrip("%U") + u"'><img width='48' height='48' src='" + unicode(iconName) + u"'></a>"
				u"</td>"
				u"<td width='30%'>"
				u"<a href='" + (unicode(execName).rstrip("%u")).rstrip("%U") + u"'><b>"+ unicode(programName) + u"</b><br><em>" + unicode(commentName) + u"</em></a>"
				u"</td>"
				)
